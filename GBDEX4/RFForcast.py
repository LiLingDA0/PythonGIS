...
from sklearn.ensemble import RandomForestClassifier  # 替换为RandomForestClassifier
...
# 创建随机森林模型
rf_model = RandomForestClassifier(n_estimators=100, random_state=42)  # 创建随机森林模型

# 训练模型
rf_model = OneVsRestClassifier(rf_model)  # 使用 OneVsRestClassifier 处理多分类问题
...
plt.title('随机森林 Confusion Matrix', fontsize=16)
...
plt.title('随机森林 Receiver Operating Characteristic')
...
plt.title('随机森林 Precision-Recall Curve')
