...
# 创建SVM模型
svm_model = SVC(kernel=KernelMethod, probability=True)  # 需要设置 probability=True 以获取概率估计

# 训练模型
svm_model = OneVsRestClassifier(svm_model)  # 使用 OneVsRestClassifier 处理多分类问题
...
with open(report_filename, 'w') as report_file:
    report_file.write("SVM-" + KernelMethod + " 模型评估结果:\n")
    report_file.write("\n")
...
plt.title('SVM - '+KernelMethod+' Confusion Matrix', fontsize=16)
...
plt.title('SVM - '+KernelMethod+' Receiver Operating Characteristic')
...
plt.title('SVM - '+KernelMethod+' Precision-Recall Curve')
